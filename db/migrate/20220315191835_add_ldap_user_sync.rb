class AddLdapUserSync < ActiveRecord::Migration[6.1]
  def up
    ...
    LdapAuthSource.where(onthefly_register: true).update_all(user_mode: LdapAuthSource.user_modes[:onthefly_register])
    remove_column :auth_sources, :onthefly_register
  end

  def down
    add_column :auth_sources, :onthefly_register, :boolean, null: false, default: false
    LdapAuthSource.where(user_mode: :synchronize).update_all(onthefly_register: true)
